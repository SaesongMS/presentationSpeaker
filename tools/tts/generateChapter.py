...
# Usage: python generateChapter.py <Article Title> <Chapter Title> <Chapter number> <Chapter Text> <speaker>
...
if len(sys.argv) >= 5:
    ...
    speaker = sys.argv[5] if len(sys.argv) == 6 else "v2/en_speaker_6"

    output_path = check_output_path(f"generations/{article_title}")

    title = ''.join(e for e in title if e.isalnum())
    generate_chapter(title, chapter_number, chapter_text, output_path, speaker)
